 ...
 * Verify Component
 * ----------------
 * Form for users to either:
 * - Claim a found item (`type=claim`)
 * - Report that they found a lost item (`type=found`)
 *
 * Behavior:
 * - Dynamically renders form based on `type` query param.
 * - Sends form data to appropriate backend API endpoint.
...
const Verify = () => {
  ...
  // Parse query parameters to determine mode and item ID
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const baseUrl = import.meta.env['PUBLIC_API_BASE_URL'] || "";
      const apiEndpoint =
        type === "claim"
          ? `${baseUrl}/api/items/verify-claim`
          : `${baseUrl}/api/items/verify-found`;
